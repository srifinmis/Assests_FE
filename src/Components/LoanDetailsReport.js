...
const LoanDetailsReport = () => {
  ...
  //   $.ajax({
  //     url: `${APIURL}/api/reports/dropdowndataloan`,
  //     method: "GET",
  //     dataType: "json",
  //     success: (data) => {
  ...
  useEffect(() => {
    const fetchDropdownData = () => {
      fetch(`${APIURL}/api/reports/dropdowndataloan`)
        .then((res) => res.json())
        .then((data) => {
          if (typeof data === "object" && data !== null) {
            ...
            localStorage.setItem("loanDropdownMetadata", JSON.stringify(metadata));
            setDropdownData(data);
          }
        })
        .catch((error) => {
    ...
    const cached = localStorage.getItem("loanDropdownMetadata");
    if (cached) {
      try {
        ...
        console.warn("Failed to parse loan dropdown cache:", err);
      }
    }
  ...
  const generateExcelReport = async () => {
    ...
    try {
      const response = await fetch(`${APIURL}/api/reports/generate-loan-details-report`, {
        method: "POST",
      ...
      XLSX.utils.book_append_sheet(workbook, newWorksheet, "LoanDetailsReport");
